...
void GlyphCache::add_glyph(SDL_Renderer* renderer, TTF_Font* font, uint32_t codepoint) {
    ...
    if (atlas_x_ + glyph_surf->w > max_width_ || atlas_y_ + glyph_surf->h > max_height_) { // TODO: Add multi-atlas
...
SDL_Rect GlyphCache::get_or_create_glyph_pos(SDL_Renderer* renderer, TTF_Font* font, uint32_t codepoint) {
    ...
    add_glyph(renderer, font, codepoint);
